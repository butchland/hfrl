...
* Solving RL Problems: 2 main approaches
   ...
   - Two Types of Policy:
      - _Deterministic_ : Policy at a given state will **ALWAYS** return the same action 
      $$ 
      a = \pi (s) 
      $$
      action = policy(state)

      - _Stochastic_: Policy outputs a **PROBALITY DISTRIBUTION** over actions
      $$
      \pi (a | s )  = P[A | s ]
      $$
      policy(actions|state) = probability distribution over the set of actions given the current state

   - Value based Methods: Learn a **VALUE FUNCTION** that maps a _STATE_ to the _EXPECTED VALUE_ of being at that _STATE_
      ...
      $$
      - _Act according to our policy_ - going to the state with the highest value
* "Deep" in Deep RL
